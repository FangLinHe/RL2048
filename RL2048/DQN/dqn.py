...
class DQN:
    ...
    def push_transition_and_optimize_automatically(self, transition: Transition) -> bool:
        self.push_transition(transition)
        if self.memory.is_full():
            losses: List[float] = []
            self.optimize_count += 1
            print(f"Optimizing - {self.optimize_count}...")
            for _ in range(self.training_params.optimize_times):
                losses.append(self.optimize_model(reset_memory=False))
